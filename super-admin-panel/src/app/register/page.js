      ...
      <div className="max-w-[1100px] w-full bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 flex overflow-hidden border border-slate-100">
        <div className="hidden lg:flex w-[45%] bg-indigo-600 p-12 flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <span className="font-bold text-xl text-white tracking-tight">Register Page</span>
          </div>

          <div className="relative z-10">
            <h1 className="text-4xl font-bold text-white mb-6 leading-tight">
              Start your journey with <span className="text-indigo-200">us.</span>
            </h1>
            <p className="text-indigo-100 text-lg">
              Join thousands of administrators managing their systems with our modern platform.
            </p>
          </div>

          <div className="relative z-10 flex -space-x-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="w-12 h-12 rounded-full border-4 border-indigo-600 bg-indigo-400 flex items-center justify-center text-white font-bold text-xs">
                U{i}
              </div>
            ))}
        ...
        <div className="flex-1 p-8 lg:p-14 flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 col-span-2 md:col-span-1">
              <label className="text-sm font-bold text-slate-700 tracking-wide">Access Role</label>
              <div className="relative group">
                <Shield className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors" size={20} />
                <select
